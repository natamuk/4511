...
public class PatientDashboardDao {
    ...
    public List<Map<String, Object>> getClinics() {
        String sql =
                "SELECT id, clinic_name, location, description, status " +
                "FROM clinic WHERE status = 1 ORDER BY sort_num, id";
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                row.put("queueEnabled", true);
                row.put("bookingEnabled", true);
    ...
    public List<Map<String, Object>> getQueueTickets(Long patientId) {
        String sql =
                "SELECT q.id, q.queue_no, q.created_time, q.status, c.clinic_name " +
                "FROM queue q " +
    ...
    public List<Map<String, Object>> getFavorites(Long patientId) {
        String sql =
                "SELECT f.id, c.id AS clinic_id, c.clinic_name, c.location " +
                "FROM patient_favorite_clinic f " +
                "JOIN clinic c ON f.clinic_name = c.clinic_name " +
                "WHERE f.patient_id = ? " +
