            ...
            <div class="max-w-6xl mx-auto space-y-10">
                ...
                <div class="glass rounded-3xl p-8">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="<%= ctx%>/patient/queue" class="text-sm text-blue-600 font-medium hover:underline">Manage Queue</a>
                    </div>
                    <div class="space-y-4">
                        <% if (queueTickets.isEmpty()) { %>
                            <div class="text-center py-12 text-gray-500"><i class="fa-solid fa-users-slash text-4xl mb-3"></i><p>You have not joined any queue today.</p></div>
                        <% } else {
                            for (Map<String, Object> qt : queueTickets) {
                                String clinicName = qt.get("clinicName") != null ? qt.get("clinicName").toString() : "Unknown Clinic";
                                String queueNo = qt.get("queueNo") != null ? qt.get("queueNo").toString() : "---";
                                String status = qt.get("status") != null ? qt.get("status").toString() : "waiting";
                                String statusClass = "";
                                ...
                                int waitMinutes = rand.nextInt(41) + 10;  // random 10-50 minutes
                        %>
                        <div class="p-5 border rounded-2xl bg-white flex flex-col md:flex-row md:items-center md:justify-between gap-4 shadow-sm">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 flex-wrap">
                                    <p class="font-bold text-xl text-gray-800"><%= clinicName %></p>
                                    <span class="px-3 py-1 rounded-full text-xs font-bold border <%= statusClass %>"><%= status.toUpperCase() %></span>
                                </div>
                                <p class="text-sm text-gray-500 mt-1">Ticket: <span class="font-mono font-bold text-indigo-600"><%= queueNo %></span></p>
                            ...
                            <a href="<%= ctx%>/patient/queue" class="px-5 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl text-center font-medium transition">View Details</a>
                        </div>
                        <% } } %>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <% } else {
                            ...
                            for (int i = 0; i < count; i++) {
                                ...
                                String timeStr = "";
                                if (n.get("time") != null) {
                                    try { timeStr = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm").format((java.util.Date) n.get("time")); } catch (Exception e) {}
                                }
                        %>
