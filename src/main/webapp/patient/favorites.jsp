...
<%
    ...
    List<Map<String, Object>> allClinics = (List<Map<String, Object>>) request.getAttribute("allClinics");
    if (favorites == null) favorites = new ArrayList<>();
    if (allClinics == null) allClinics = new ArrayList<>();
...
<div class="flex h-screen overflow-hidden relative">
    <div class="w-72 glass shadow-2xl flex flex-col border-r border-white/50 z-40">
        ...
        <div class="p-6 flex-1 flex flex-col">
            ...
            <nav class="space-y-1">
                ...
                <a href="<%= ctx %>/patient/favorites" class="nav-item flex items-center gap-3 px-5 py-3 rounded-2xl active"><i class="fa-solid fa-star w-5"></i><span>Favorite Clinics</span></a>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="glass border-b px-8 py-4 flex justify-between items-center z-10">
            <div>
                <h2 class="text-2xl font-semibold">Favorite Clinics</h2>
            ...
            <div class="glass rounded-3xl p-8 max-w-4xl mx-auto">
                ...
                <div id="favorites-list" class="space-y-4">
                    <% if (favorites.isEmpty()) { %>
                        <div class="text-center py-12 text-gray-500"><i class="fa-regular fa-heart text-4xl mb-3"></i><p>No favorite clinics added yet.</p></div>
                    <% } else {
                        for (Map<String, Object> fav : favorites) {
                            Long clinicId = (Long) fav.get("clinicId");
                            String clinicName = "";
                            String clinicLocation = "";
                            for (Map<String, Object> c : allClinics) {
                                if (clinicId.equals(c.get("id"))) {
                                    clinicName = (String) c.get("name");
                                    clinicLocation = (String) c.get("location");
                                    break;
                                }
                            }
                    %>
                        <div class="p-5 border rounded-2xl bg-white flex justify-between items-center shadow-sm">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1"><i class="fa-solid fa-location-dot mr-1"></i> <%= clinicLocation %></p>
                            </div>
                            <div class="flex gap-3">
                                <a href="<%= ctx %>/patient/book" class="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100">Book</a>
                                <form action="<%= ctx %>/patient/toggle-favorite" method="post" style="display:inline;" onsubmit="return confirm('Remove from favorites?')">
                                    <input type="hidden" name="clinicId" value="<%= clinicId %>">
                                    <button type="submit" class="px-4 py-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-100"><i class="fa-solid fa-heart"></i></button>
                                </form>
                            </div>
                        </div>
                    <% } } %>
                </div>
                <div class="mt-6"><a href="<%= ctx %>/patient/book" class="btn bg-blue-600 text-white px-4 py-2 rounded-xl">Browse Clinics</a></div>
            </div>
        </div>
    </div>
</div>
