        ...
        <script>
            ...
            function setActiveNav(page) {
                ...
                document.querySelectorAll(`.nav-item`).forEach(b => {
                    ...
                    if (onclick.includes(`loadPage('${page}')`))
            ...
            function initSettingsFragment() {
                ...
                const clinicsSection = document.getElementById('walkin-clinics-section');
                const saveBtn = document.getElementById('save-settings-btn');
                const clinicsContainer = document.getElementById('clinics-list');

                function updateVisibility() {
                    if (queueToggle && clinicsSection) {
                        clinicsSection.style.display = queueToggle.checked ? 'block' : 'none';
                    }
                }

                if (queueToggle) {
                    queueToggle.addEventListener('change', updateVisibility);
                }

                function loadSettings() {
                    fetch(CONTEXT_PATH + '/admin/settingsData', {credentials: 'same-origin'})
                            .then(res => {
                                if (!res.ok)
                            ...
                            .then(data => {
                                ...
                                var stMax = document.getElementById('st-max');
                                var stCancel = document.getElementById('st-cancel');
                                var stApproval = document.getElementById('st-approval');

                                if (stMax)
                                    stMax.value = settings.max_active_bookings_per_patient || settings.max_active_bookings || '3';
                                if (stCancel)
                                    stCancel.value = settings.cancel_deadline_hours || '24';
                                if (queueToggle)
                                    queueToggle.checked = (settings.same_day_queue_enabled === '1');
                                if (stApproval)
                                    stApproval.checked = (settings.booking_approval_required === '1');
                                updateVisibility();

                                var enabledClinics = (settings.walkin_enabled_clinics || '').split(',').filter(s => s.trim().length > 0);
                                var clinicsList = data.clinics || [];
                                if (clinicsContainer) {
                                    clinicsContainer.innerHTML = '';
                                    if (!clinicsList || clinicsList.length === 0) {
                                        clinicsContainer.innerHTML = '<div class="text-gray-400 text-sm col-span-2">No clinic data available.</div>';
                                    } else {
                                        clinicsList.forEach(function (clinic) {
                                            ...
                                            cb.type = 'checkbox';
                                            cb.className = 'walkin-clinic-checkbox';
                                            cb.value = clinic.id;
                                            if (enabledClinics.includes(String(clinic.id)))
                                                cb.checked = true;
                                            label.appendChild(cb);
                                            label.appendChild(document.createTextNode(clinic.name));
                                            clinicsContainer.appendChild(label);
                                        });
                                    }
                                }
                            })
                            .catch(err => {
                                ...
                                if (clinicsContainer)
                                    clinicsContainer.innerHTML = '<div class="text-red-500 text-sm">Failed to load clinics.</div>';
                            });
                }

                function saveSettings() {
                    var maxBookingsEl = document.getElementById('st-max');
                    var cancelHoursEl = document.getElementById('st-cancel');
                    var approvalEl = document.getElementById('st-approval');

                    var maxBookings = maxBookingsEl ? maxBookingsEl.value : '';
                    var cancelHours = cancelHoursEl ? cancelHoursEl.value : '';
                    var walkinEnabled = queueToggle && queueToggle.checked ? '1' : '0';
                    var approvalRequired = approvalEl && approvalEl.checked ? '1' : '0';
                    var selectedClinics = [];
                    document.querySelectorAll('.walkin-clinic-checkbox:checked').forEach(cb => selectedClinics.push(cb.value));
                    var formData = new URLSearchParams();
                    formData.append('max_active_bookings_per_patient', maxBookings);
                    formData.append('cancel_deadline_hours', cancelHours);
                    formData.append('same_day_queue_enabled', walkinEnabled);
                    formData.append('booking_approval_required', approvalRequired);
                    formData.append('walkin_enabled_clinics', selectedClinics.join(','));

                    fetch(CONTEXT_PATH + '/admin/settings/save', {
                        method: 'POST',
                        ...
                        credentials: 'same-origin'
                            ...
                            .catch(err => {
                                ...
                                Swal.fire('Error', msg || 'Save failed', 'error');
                            });
                }
            ...
            function loadPage(page) {
                ...
                fetch(url, {credentials: 'same-origin'})
                        ...
                        .then(html => {
                            ...
                            if (page === 'settings' && typeof initSettingsFragment === 'function') {
                                try {
                                    initSettingsFragment();
                                } catch (e) {
                                    console.error('initSettingsFragment error:', e);
                                }
                            }
                        ...
                        .catch(err => {
                            ...
                            c.innerHTML = '<div class="p-8 text-center text-red-500">Failed to load content (' + (err.message || err) + ').</div>';
                            c.style.opacity = '1';
                        });
            }
