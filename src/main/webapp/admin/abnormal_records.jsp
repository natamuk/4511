...
<%
    ...
    List<Map<String, Object>> highNoShowDoctors = dao.getDoctorsWithHighNoShow(5);
    
    // === Send notifications to doctors with high no-show count (once per day) ===
    String alertKey = "HighNoShowAlertSentDate";
    String todayStr = java.time.LocalDate.now().toString();
    String lastSent = (String) session.getAttribute(alertKey);
    if (lastSent == null || !lastSent.equals(todayStr)) {
        dao.sendNotificationsForHighNoShowDoctors();
        session.setAttribute(alertKey, todayStr);
    }
...
<div class="flex h-screen overflow-hidden">
    <aside class="w-80 glass shadow-2xl flex flex-col border-r border-white/50 fixed md:relative h-full z-40">
        ...
        <div class="p-6 flex-1 flex flex-col overflow-y-auto">
            ...
            <nav class="space-y-1">
                ...
                <a href="${pageContext.request.contextPath}/admin/abnormal_records.jsp" class="nav-item active"><i class="fa-solid fa-exclamation-triangle"></i><span>Abnormal Records</span></a>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="glass border-b px-8 py-4 flex justify-between items-center">
            <h2 class="text-2xl font-semibold">Abnormal Records</h2>
            <today:today/>
        </header>
            ...
            <div class="space-y-8">
                ...
                <div class="glass p-6 rounded-3xl">
                    ...
                    <c:if test="${not empty repeatNoShow}">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-gray-100"><tr><th class="p-3">Patient</th><th class="p-3">Phone</th><th class="p-3">No-show Count</th></tr></thead>
                            <tbody>
                                <c:forEach var="item" items="${repeatNoShow}">
                                    <tr class="border-b"><td class="p-3">${item.patient_name}</td><td class="p-3">${item.phone}</td><td class="p-3 font-bold text-red-600">${item.no_show_count}</td></tr>
                                </c:forEach>
                            </tbody>
                        </table>
                    </c:if>
                ...
                <div class="glass p-6 rounded-3xl">
                    ...
                    <c:if test="${not empty frequentCancel}">
                        <table class="w-full text-left">
                            <thead class="bg-gray-100"><tr><th class="p-3">Patient</th><th class="p-3">Cancel Count (7d)</th><th class="p-3">Last Cancel Date</th></tr></thead>
                            <tbody>
                                <c:forEach var="item" items="${frequentCancel}">
                                    <tr class="border-b"><td class="p-3">${item.patient_name}</td><td class="p-3">${item.cancel_count}</td><td class="p-3">${item.last_cancel_date}</td></tr>
                                </c:forEach>
                            </tbody>
                        </table>
                    </c:if>
                ...
                <div class="glass p-6 rounded-3xl">
                    ...
                    <c:if test="${not empty highNoShowDoctors}">
                        <table class="w-full text-left">
                            <thead class="bg-gray-100"><tr><th class="p-3">Doctor</th><th class="p-3">Department</th><th class="p-3">No-show Count</th></tr></thead>
                            <tbody>
                                <c:forEach var="doc" items="${highNoShowDoctors}">
                                    <tr class="border-b"><td class="p-3">${doc.doctor_name}</td><td class="p-3">${doc.dept_name}</td><td class="p-3">${doc.no_show_count}</td></tr>
                                </c:forEach>
                            </tbody>
                        </table>
                    </c:if>
