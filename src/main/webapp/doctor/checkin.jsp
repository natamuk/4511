...
<%
    ...
    List<Map<String, Object>> pending = (List<Map<String, Object>>) request.getAttribute("pendingConsultations");
...
<html>
<head>
    <meta charset="UTF-8"><title>Doctor - Consultation</title>
    ...
    <div class="w-80 glass shadow-2xl flex flex-col border-r border-white/50 z-40 fixed h-full">
        ...
        <div class="p-6 flex-1 flex flex-col overflow-y-auto">
            ...
            <nav class="flex flex-col gap-1">
                ...
                <a href="<%= ctx %>/doctor/checkin" class="nav-item flex items-center gap-3 px-5 py-3 rounded-2xl active"><i class="fa-solid fa-clipboard-check w-5"></i><span>Consultation</span></a>
    ...
    <div class="flex-1 flex flex-col min-w-0 ml-80">
        <div class="flex-1 overflow-auto p-4 md:p-8">
            <div class="max-w-6xl mx-auto">
                <h2 class="text-2xl font-bold mb-6">Pending Consultations</h2>
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <% for (Map<String, Object> p : pending) {
                                ...
                                Long id = idNum == null ? 0L : idNum.longValue();
                                String ticketNo = (String) p.get("ticketNo");
                                String patient = (String) p.get("patient");
                                String service = (String) p.get("service");
                                Number statusNum = (Number) p.get("statusCode");
                                int statusCode = statusNum == null ? 0 : statusNum.intValue();
                            %>
                            <tr>
                                <td><%= ticketNo %></td>
                                <td><%= patient %></td>
                                <td><%= service %></td>
                                <td><%= statusCode==3?"Called":(statusCode==4?"Consulting":"") %></td>
                                <td>
                                    <button onclick="openModal('<%= source %>', <%= id %>, '<%= patient %>')" class="btn-primary">Fill Record</button>
                                    <% if (statusCode == 4) { %>
                                        <button onclick="completeConsultDirect('<%= source %>', <%= id %>)" class="btn-secondary ml-2">Complete</button>
                                    <% } %>
                                </td>
                            </tr>
                            <% } %>
                            <% if (pending.isEmpty()) { %><tr><td colspan="5" class="text-center py-4">No pending consultations.</td></tr><% } %>
                        </tbody>
                    </table>
                </div>
            </div>
...
<!-- Modal -->
<div id="modal" style="display:none; position:fixed; top:20%; left:30%; background:white; padding:20px; border:1px solid #ccc; z-index:1000; border-radius:1rem; width:400px;">
    <h3 class="text-xl font-bold mb-4">Consultation Record</h3>
    <input type="hidden" id="modalSource"><input type="hidden" id="modalId">
    <label class="block mb-2">Diagnosis: <textarea id="diagnosis" rows="2" cols="30" class="border p-2 w-full rounded"></textarea></label>
    <label class="block mb-2">Prescription: <textarea id="prescription" rows="2" cols="30" class="border p-2 w-full rounded"></textarea></label>
    <label class="block mb-2">Advice: <textarea id="advice" rows="2" cols="30" class="border p-2 w-full rounded"></textarea></label>
    <div class="flex gap-2 mt-4">
        <button onclick="submitConsult()" class="bg-blue-600 text-white px-4 py-2 rounded">Submit & Complete</button>
        <button onclick="closeModal()" class="bg-gray-300 px-4 py-2 rounded">Cancel</button>
    </div>
</div>
<script>
function openModal(source, id, patientName) {
...
function submitConsult() {
    let source = document.getElementById('modalSource').value;
    ...
    fetch('<%= ctx %>/doctor/consultation', {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: `regId=${id}&diagnosis=${encodeURIComponent(diagnosis)}&prescription=${encodeURIComponent(prescription)}&advice=${encodeURIComponent(advice)}&source=${source}`
    }).then(res=>res.json()).then(data=>{ if(data.success) location.reload(); else alert(data.message); });
}
function completeConsultDirect(source, id) {
