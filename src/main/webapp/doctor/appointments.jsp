...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .queue-card { transition: transform 0.15s ease, box-shadow 0.15s ease; }
        .queue-card:hover { transform: translateY(-3px); box-shadow: 0 12px 24px rgba(0,0,0,0.06); }
        ...
        .btn, .btn-red { padding: 0.25rem 0.75rem; border-radius: 0.5rem; font-size: 0.875rem; cursor: pointer; border: none; }
    ...
    <div class="w-80 glass shadow-2xl flex flex-col border-r border-white/50 z-40 fixed h-full">
        ...
        <div class="p-6 flex-1 flex flex-col overflow-y-auto">
            ...
            <nav class="flex flex-col gap-1">
                ...
                <a href="<%= ctx %>/doctor/appointments" class="nav-item flex items-center gap-3 px-5 py-3 rounded-2xl active"><i class="fa-solid fa-calendar-days w-5"></i><span>Appointments</span></a>
                ...
                <div class="card">
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            <thead>
                                <tr><th>Date</th><th>Ticket No</th><th>Patient</th><th>Service</th><th>Status</th><th>Action</th></tr>
                            </thead>
                            <tbody>
                                <% for (Map<String, Object> a : appointments) {
                                    Number statusNum = (Number) a.get("statusCode");
                                    int statusCode = statusNum == null ? 0 : statusNum.intValue();
                                    Number idNum = (Number) a.get("id");
                                    int id = idNum == null ? 0 : idNum.intValue();
                                    String date = a.get("date") != null ? a.get("date").toString() : "";
                                    String ticketNo = a.get("ticketNo") != null ? a.get("ticketNo").toString() : "";
                                    String patient = a.get("patient") != null ? a.get("patient").toString() : "";
                                    String service = a.get("service") != null ? a.get("service").toString() : "";
                                %>
                                <tr>
                                    <td><%= date %></td>
                                    <td><%= ticketNo %></td>
                                    ...
                                    <td>
                                        <span class="status-badge 
                                            <%= statusCode==1?"status-pending":(statusCode==3?"status-arrived":(statusCode==4?"status-approved":(statusCode==5?"status-completed":"status-cancelled"))) %>">
                                            <%= statusCode==1?"Pending":(statusCode==3?"Checked In":(statusCode==4?"Consulting":(statusCode==5?"Done":"Rejected"))) %>
                                        </span>
                                    </td>
                                    <td>
                                        <% if (statusCode == 1) { %>
