                    ...
                    <div class="space-y-4">
                        ...
                        <% } else {
                            for (Map<String, Object> item : appointments) {
                                ...
                                Long clinicIdObj = (Long) item.get("clinicId");
                                String clinicIdStr = clinicIdObj != null ? clinicIdObj.toString() : "";
                        %>
                            <div class="p-6 border rounded-3xl bg-white flex flex-col md:flex-row md:items-center md:justify-between gap-6 shadow-sm hover:shadow-md transition">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 md:w-auto w-full pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                                    <% if (isBooked) { %>
                                        <% if (canReschedule && clinicIdObj != null) { %>
...
<script>
    ...
    document.querySelectorAll('.reschedule-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const clinicIdVal = btn.dataset.clinicId;
            if (!clinicIdVal) {
                Swal.fire('Error', 'Clinic information missing, cannot reschedule', 'error');
                return;
            }
            currentRegId = btn.dataset.id;
            currentClinicId = clinicIdVal;
    ...
    document.getElementById('confirmRescheduleBtn').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${contextPath}/patient/reschedule-booking`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `registrationId=${currentRegId}&newDate=${newDate}&newTime=${newSlot}`
            });
