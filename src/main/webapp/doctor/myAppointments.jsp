...
<div class="flex h-screen overflow-hidden relative">
    <!-- 左側導航欄（內容同前，active 為 My Schedule） -->
            ...
            <div class="max-w-6xl mx-auto">
                ...
                <div class="card overflow-x-auto">
                    <table class="min-w-full">
                        <thead><tr><th>Ticket</th><th>Patient</th><th>Time</th><th>Status</th></tr></thead>
                        <tbody>
                            <% for (Map<String, Object> a : myApps) { %>
                            <tr>
                                <td><%= a.get("ticketNo") %></td>
                                <td><%= a.get("patient") %></td>
                                <td><%= a.get("timeSlot") %></td>
                                <td><%= a.get("status") %></td>
                            </tr>
                            <% } %>
                            <% if (myApps.isEmpty()) { %><tr><td colspan="4" class="text-center py-4">No appointments today.</td></tr><% } %>
                        </tbody>
                    </table>
                </div>
            </div>
